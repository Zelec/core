...
{self, ...}: {
    ...
    packages = {
      ...
      in
        pkgs.buildGoModule rec {
          ...
          preBuild = ''
            tar xzf ${zuiSrc} -C pkg/extensions/
          '';
          tags = [
            "sync"
            "search"
          ...
          ldflags = [
            "-s"
            "-w"
            "-X zotregistry.dev/zot/v2/pkg/api/config.ReleaseTag=v${version}"
  ...
  flake.nixosModules.services-zot = {
    ...
    with lib; let
      ...
      package = self.packages.${pkgs.stdenv.hostPlatform.system}.zot;
      ...
      generateHtpasswd = let
        ...
        commands =
          imap0 (
            ...
            in "${pkgs.apacheHttpd}/bin/htpasswd ${flags} ${escapeShellArg htpasswdPath} ${escapeShellArg name} < ${escapeShellArg (toString u.passwordFile)}"
          )
          userNames;
      in
        concatStringsSep "\n" (["rm -f ${escapeShellArg htpasswdPath}"] ++ commands ++ ["chmod 600 ${escapeShellArg htpasswdPath}"]);
    in {
      options.services.zot = with types;
        mkOption {
